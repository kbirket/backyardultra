...
(function(){
  ...
  const updateWarning=()=>{
    ...
    const start=new Date(r['Start Time']);let loop=Number(r['Current Loop']||1);
  ...
  const updateHome=()=>{
    ...
    if(!qs('#raceBrainCard')){
      ...
      card.innerHTML='<div class="brain-icon" aria-hidden="true">✦</div><div class="section-kicker">RACE BRAIN</div><div id="brainTitle" class="brain-title">NOTHING NEEDS YOUR ATTENTION</div><div id="brainMeta" class="brain-meta">Stay steady. The next bell is the only thing that matters.</div><div class="brain-signal" aria-hidden="true"><i></i><i></i><i></i></div>';
