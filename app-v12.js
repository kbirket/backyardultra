...
(function(){
  ...
  async function getGear(){const r=await fetch('/api/race',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'getAll'})});const d=await r.json();if(!r.ok||d.error)throw Error(d.error||`Request failed (${r.status})`);return Array.isArray(d.gear)?d.gear:[]}
  async function loadGear(){
    ...
    try{
      ...
      el.querySelectorAll('[data-gear-id]').forEach(btn=>btn.onclick=async()=>{const id=btn.dataset.gearId;btn.disabled=true;try{const items=await getGear(),g=items.find(x=>x.id===id);if(!g)return;const done=g.fields?.Done===true;const r=await fetch('/api/race',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'setGearDone',id,done:!done})});const d=await r.json();if(!r.ok||d.error)throw Error(d.error||'Could not update gear');await loadGear()}catch(e){console.error(e)}finally{btn.disabled=false}});
